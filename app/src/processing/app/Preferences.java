  ...
  static protected void init(String commandLinePrefs) {
    ...
    try {
      ...
      Base.showError(null, "Could not read default settings.\n" +
                           "You'll need to reinstall " + Base.PROGRAM_NAME + ".", e);
    }
      ...
      if (!preferencesFile.exists()) {
        ...
        try {
          ...
          Base.showError("Error reading preferences",
                         ...
                         " and restart " + Base.PROGRAM_NAME + ".", ex);
        }
      }
...
//      }
      ...
      load(new FileInputStream(new File(Base.getHardwareFolder(), "boards.txt")),
        ...
        Base.showError("Error reading board definitions",
		       ...
		       new File(Base.getHardwareFolder(), "boards.txt").getAbsolutePath() + "). " +
                       "Please re-download or re-unzip " + Base.PROGRAM_NAME + ".\n", ex);
    }
  ...
  public Preferences() {
    ...
    */
    ...
    label = new JLabel("  (requires restart of " + Base.PROGRAM_NAME + ")");
    box.add(label);
    pain.add(box);
    ...
    if (Base.isWindows()) {
      autoAssociateBox =
        new JCheckBox("Automatically associate .pde files with " + Base.PROGRAM_NAME + "");
    ...
    label = new JLabel("(edit only when " + Base.PROGRAM_NAME + " is not running)");
    pain.add(label);
    d = label.getPreferredSize();
    label.setForeground(Color.gray);
    label.setBounds(left, top, d.width, d.height);
